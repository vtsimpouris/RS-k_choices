...
class RadixSpline {
 public:
  ...
  uint64_t CalculateSplineIndex(const KeyType key) const {
    // Narrow search range using radix table.
    if (key <= min_key_) return ULONG_MAX-1;
    if (key >= max_key_) return ULONG_MAX-1;
    ...
    if (end - begin < 32) {
      ...
      while (spline_points_[current].x < key) ++current;
          ...
          current = std::distance(spline_points_.begin(), lb);
    }
  ...
  double averageSearchCost() const {
    double counter = 0;
    //std::cout << "end - begin " << end - begin << std::endl;
    for(int i = 0; i < radix_table_.size() - 1; i++){
        counter += radix_table_[i + 1] - radix_table_[i];
    }
    double avg_search_cost = log2(counter/num_radix_bits_);
