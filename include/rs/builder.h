...
class Builder {
 public:
  Builder(KeyType min_key, KeyType max_key, size_t num_radix_bits = 25, size_t max_error = 2)
      : min_key_(min_key),
        max_key_(max_key),
  ...
  void AddKey(KeyType key) {
    ...
    //std::cout << "prev pos: " << prev_position_ + 1 << std::endl;
    AddKey(key, prev_position_ + 1);
  }

  void AddKey_ext_pos(KeyType key, long int position) {
    if (curr_num_keys_ == 0) {
      AddKey(key, position);
      return;
    }
  ...
  double checkCDF(KeyType key,int equality = 0) {
    if(equality > 0){
      return checkCDF(key, prev_position_ + 1,equality);
    }
    return checkCDF(key, prev_position_ + 1,0);
  }
...
double checkCDF(KeyType key, double position, int equality) {
    ...
    if (((ComputeOrientation(upper_limit_x_diff, upper_limit_y_diff, x_diff, y_diff) != Orientation::CW)
      ...
      if (ComputeOrientation(lower_limit_x_diff, lower_limit_y_diff, x_diff, lower_y_diff) == Orientation::CCW) {
        shrink = (lower_y- last.y)/(key - last.x) - (lower_limit_.y - last.y)/(lower_limit_.x - last.x);
        margin = (upper_y_pr - lower_y);
        if(update_upper){
          margin = upper_y - lower_y;
          shrink += (upper_limit_.y - last.y)/(upper_limit_.x - last.x) - (upper_y- last.y)/(key - last.x);
        }
 ...
 private:
  ...
  static double ComputeOrientation_expr(const double dx1, const double dy1, const double dx2, const double dy2) {
    const double expr = std::fma(dy1, dx2, -std::fma(dy2, dx1, 0));
   return expr;
  };
  ...
  void FinalizeRadixTable() {
    ...
    //std::cout << spline_points_.size() << std::endl;
